...
class firstCollectionViewCell: UICollectionViewCell {
    ...
    public func configure(with model: Model){
        ...
        if model.isMine {borderView.layer.borderWidth = 0}
        
        if model.storyIsWatch == true {
            self.borderView.layer.borderColor = UIColor.gray.cgColor
        }
        else {
            self.borderView.layer.borderColor = CGColor(red: 8.0/255.0, green: 255.0/255.0, blue: 114.0/255.0, alpha: 1.0)
        }
